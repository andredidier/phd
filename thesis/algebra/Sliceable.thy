...
class sliceable = 
  ...
  assumes empty_seq_inter [simp]: 
    "disjoint x \<Longrightarrow> c \<le> k \<Longrightarrow> empty_inter (x\<dagger>0..c) (x\<dagger>k..(#x))"
  ...
  assumes disjoint_slice_suc: 
    "disjoint x \<Longrightarrow> i\<noteq>j \<Longrightarrow> i < (#x) \<Longrightarrow> j < (#x) \<Longrightarrow> 
    x\<dagger>i..(Suc i) \<noteq> x\<dagger>j..(Suc j)"
  assumes disjoint_slice[simp]: "disjoint x \<Longrightarrow> disjoint (x\<dagger>i..j)"
  assumes forall_slice_implies_eq: "(#x) = (#y) \<and> (\<forall>i j. (x\<dagger>i..j) = 
  ...
  *)
notation (latex output) slice  ("(3_\<^bsub>[_.._]\<^esub>)" [80,80,80] 80)

text {* Teste @{term "slice x i j"}*}

definition slice_right :: "'a::sliceable \<Rightarrow> nat \<Rightarrow> 'a" ("(2_\<dagger>.._)" [80,80] 80)
where "slice_right x i = x\<dagger>0..i"

notation ("latex") slice_right  ("(2_\<^bsub>[.._]\<^esub>)" [80,80] 80)

(*text {* The notation for @{term "slice_right x i"} is @{term "x\<^bsub>[..i]\<^esub>"}  *}*)
...
lemma (in sliceable) slice_left_disjoint[simp]: "disjoint xs \<Longrightarrow> 
...
theorem (in sliceable) empty_seq_sliced_inter [simp]: 
  "disjoint x \<Longrightarrow> b \<le> i \<Longrightarrow> j \<le> a \<Longrightarrow> i \<le> j \<Longrightarrow> a \<le> size x \<Longrightarrow> 
...
proof-
  ...
  hence 2: "empty_inter (?l\<dagger>0..(i-b)) (?l\<dagger>(j-b)..(#?l))" 
...
corollary (in sliceable) slice_left_slice_right_absorb: 
  "(l\<dagger>i..)\<dagger>..j = l\<dagger>i..(i+j)"
unfolding slice_left_def slice_right_def
proof -
...
lemma (in sliceable) slice_f_min_neutral: 
  "(P (l\<dagger>i..(min f k)) \<and> f \<le> k) \<longleftrightarrow> (P (l\<dagger>i..f) \<and> f \<le> k)"
by linarith

lemma (in sliceable) slice_i_min_neutral: 
  "(P (l\<dagger>(min i k)..f) \<and> i \<le> k) \<longleftrightarrow> (P (l\<dagger>i..f) \<and> i \<le> k)"
by linarith

lemma (in sliceable) slice_i_min_neutral_lt: 
  "(P (l\<dagger>(min k i)..f) \<and> i < k) \<longleftrightarrow> (P (l\<dagger>i..f) \<and> i < k)"
by linarith
...
lemma (in sliceable) slice_f_max_neutral: 
  "(P (l\<dagger>i..(max f k)) \<and> f \<ge> k) \<longleftrightarrow> (P (l\<dagger>i..f) \<and> f \<ge> k)"
by (metis max.orderE)

lemma (in sliceable) slice_i_max_neutral: 
  "(P (l\<dagger>(max i k)..f) \<and> i \<ge> k) \<longleftrightarrow> (P (l\<dagger>i..f) \<and> i \<ge> k)"
by (metis max.orderE)
...
corollary (in sliceable) forall_disjoint_slice_suc:
  "\<forall> i j . (disjoint x \<and> i\<noteq>j \<and> i < (#x) \<and> j < (#x)) \<longrightarrow> 
    (x\<dagger>i..(Suc i) \<noteq> x\<dagger>j..(Suc j))"
