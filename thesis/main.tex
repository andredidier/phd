...
The algebra created in this work defines the NOT operator and allows its use\amnote{without introducing incoerence?} \adnote{Prefiro deixar assim mesmo...}, as we show in \cref{chap:strategy}.
\end{sloppypar}
...
\begin{rqenum}[series=researchquestion]
  \item Is there a mathematical model that is set-based and similar to \ac{FBA}?\label{question:mathematical-model}
\end{rqenum}
%
...
%
In this version of the thesis we propose the theory that answers \cref{question:mathematical-model,question:ordering-representation,question:gap}. \Cref{question:analyses,question:efficiency} are not answered in this version of the thesis.
...
\Cref{fig:strategy-overview} shows an overview of the strategy.
...
The fault events sequences are then mapped to \theac{algebra}, which have denotational semantics based on sets of lists.
...
\begin{contrenum}[series=contributions]
  ...
  \item Reuse \simulink models, obtaining fault event sequences and mapping to \ac{algebra}---partially done, see \cref{chap:strategy};
  \item Reason about faults modelling in \ac{activation}, to obtain formal expressions of critical failures (top-event failures)---see discussion in~\cref{sec:activation};
...
\Cref{part:contributions} describes the results: \cref{chap:strategy} presents our strategy, \cref{chap:case-study} the case study and the application of the proposed strategy, and we present our conclusions and future work in \cref{part:final-remarks}.
...
We explain how these methods combine in \cref{chap:strategy}.
...
In \cref{chap:strategy} we abstract events relation in continuous time as an \emph{exclusive before} relation, based on fault \emph{detection} (it is similar---at least implicitly---to what is reported in~\cite{WP2009,MRL2011}).
...
To show how these traces become failure expression, let us abbreviate fault names as:\amnote{Verificar espaçamento deste parágrafo.}
%
\begin{snippetcspm}[2]
...
\begin{Prop}[\hl{Soundness and completeness} of \ac{algebra}]
...
The effort shown in the ``Weeks'' column is the estimated remaining time to finish the task.
...
\IBGEtab{%
  ...
  \begin{tabular}{p{9.9cm}ccc}
    \toprule
    Task & Start & Weeks & Weeks total\\
    \midrule \midrule
    Qualification & 04/04/2016 & 0 & 0\\
    \midrule
    Elaborate a theory for the \ac{activation} & 04/04/2016 & 7 & 0\\
    \midrule
    Elaborate a theory for the acceptance criteria & 23/05/2016 & 4 & 7\\
    \midrule
    Prepare a paper about \ac{activation} and acceptance criteria & 20/06/2016 & 4 & 11\\
    \midrule
    Submit paper about \ac{activation} and acceptance criteria & 18/07/2016 & 0 & 15\\
    \midrule
    Prove soundness and completeness theorems for the \ac{DNF} of \ac{algebra} & 18/07/2016 & 1 & 15\\
    \midrule
    Define the mapping rules from traces to \ac{algebra} & 03/10/2016 & 2 & 26\\
    \midrule
    Demonstrate the relations of \ac{NOT} and \ac{XBefore} and other operators & 17/10/2016 & 2 & 28\\
    \midrule
    Define the conditions that cause non-coherent analysis with \ac{NOT} & 31/10/2016 & 2 & 30\\
    \midrule
    Write the results in the thesis & 14/11/2016 & 8 & 32\\
    \midrule
    Prepare thesis' defence & 09/01/2017 & 4 & 40\\
    \midrule
    Defence & 06/02/2017 & 0 & 44\\
    \bottomrule
  \end{tabular}%
}{%
}
...
\begin{apendicesenv}
\partapendices
\chapter{Formal proofs in \isabellehol}
