...
The reduction of structure expressions in \ac{TFT} is based in a combination of: (i) algebraic reduction---that can lead to an ever-growing rules application---, (ii) modularisation of independent subtrees (not always subtree are independent), and (iii) \ac{TDT}---which are limited to seven basic events, due to exponential grow of the tree.
...
As both \ac{DTMC} and \ac{CTMC} are state-based, they suffer the state-space explosion problem~\cite{BKK+2003,BHH+2003}.
...
``Faults injection'' block in \cref{fig:strategy-overview}, path A, is obtained from part of the work reported in~\cite{DM2012,Didier2012}.
...
Given the flexibility of \ac{activation} notation, it can be used directly (path B) to model faults formally, reasoning about basic fault events and top-event failures.
...
\adnote{The work reported in SAE1996b states that Markov chains is more appropriate to represent several states than FTs. So the use of patterns to propose changes would be more efficient if used for DFT.}
