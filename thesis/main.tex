...
\begin{description}
  ...
  Also, the probability of failure of replicated systems can be evaluated, so a homogeneous replication could be replaced by a heterogeneous one, reducing the probability of failure (in general)\todo{Citar trabalho sobre redução de probabilidade com replicação heterogênea}.
\end{description}
...
As \acp{DFT} originally employs \ac{DTMC} analysis, the methods to analyse are generally state-based\todo{cite all dft works that are state-based}.
...
They are still referred as \ac{BDD}, but the more spread version is the \ac{ROBDD}, which is an optimised version, based on Shannon's method (describe in \FThandbook).
