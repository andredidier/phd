...
%
  = & \left(S \land B_2 \land \lnot B_1\right) \lor 
  \left(B_1 \land B_2 \land \lnot F\right) \lor
  \left(\xbefore{B_1}{F} \land \lnot B_2\right) \lor \nonumber \\
...
The expression $PowerSource_i$ is healthy.
...
Composing the monitor with the two power sources, we obtain the \ac{activation} expression of a power supply subsystem $System_{\mathrm{bool}}$:
%
\begin{align*}
...
We obtain the following expression for the monitor, now using the \algebra:

%
\begin{equation}
\label{eq:monitor-algebra}
\begin{split}
Monitor_{\algebra} &\left(in_1, in_2\right) = \\
  & \modes{in_1}{\predicate{in_1}{\Nominal{X}} \land \lnot F} \cup\\
  & \modes{in_2}{\lnot \predicate{in_1}{\Nominal{X}} \land \lnot F} \cup\\
  & \modes{in_2}{\predicate{in_1}{\Nominal{X}} \land F} \cup\\
...
Thus the operands satisfy all temporal properties (\cref{def:tempo1,def:tempo2,def:tempo3,def:tempo4}) and events independence (\cref{eq:independent-events}), thus the law shown in \cref{thm:xbefore-sup-equiv-inf} is valid.
...
%
\begin{align*}
System_{\algebra} = & Monitor_{\algebra} \left(PowerSource_1, PowerSource_2\right) \\
%  = & \modes{in_1}{\lnot B_1 \land \lnot F} \cup\\
%  & \modes{in_2}{\lnot \lnot B_1 \land \lnot F} \cup\\
%  & \modes{in_2}{\lnot B_1 \land F} \cup\\
...
\[
\begin{split}
\healthinessfun[1]{System_{\algebra}} = &\\
  & \left\{ 
      \left(\lnot B_1 \land \lnot F, \Nominal{12V}\right), 
      \left(B_2 \land B_1 \land \lnot F, LP\right),\right.\\
  &   \left(\lnot B_2 \land B_1 \land \lnot F, \Nominal{12V}\right), 
...
\[
\begin{split}
\healthiness[3] \circ \healthiness[1] &\left(System_{\mathrm{\algebra}}\right)\\
=  & \left\{ 
      \left(
        \begin{split}
          \left(\lnot B_1 \land \lnot F\right) \lor\\
...
= & \left\{
  ...
  & \left.  
    \left(\left(B_1 \land B_2\right) \lor \left(\lnot B_1 \land B_2 \land F\right) \lor \left(\lnot B_2 \land \xbefore{B_1}{F}\right), 
      LP\right)
  \right\}
\end{split}
\]
...
Simplifying \Boolean operators as usual, the XBefore expression:
%
\begin{align*}
  \lnot B_2 \land \xbefore{F}{B_1} &\lor \lnot B_2 \land \xbefore{B_1}{F}\\
...
Thus:
\[
\healthiness[2] \circ \healthiness[3] \circ \healthiness[1]\left(System_{\mathrm{\algebra}}\right) =
\healthiness[3] \circ \healthiness[1]  \left(System_{\mathrm{\algebra}}\right)
\]
...
\begin{split}
  \healthinesscmd\left(System_{\algebra}\right) = & \left\{
    \left(\left(\lnot B_1 \land \lnot B_2\right) \lor 
      \lnot F \land \left(\lnot B_1 \lor \lnot B_2\right) \lor \right.\right.\\
  & \left. \lnot B_2 \land \xbefore{F}{B_1}, \Nominal{12V}\right), \\
  & \left(\left(B_1 \land B_2\right) \lor \left(\lnot B_1 \land B_2 \land F\right) \lor \right.\\
...
\[
\predicate{System_{\mathrm{\algebra}}}{LP} \iff 
  \left(B_1 \land B_2\right) \lor 
  \left(\lnot B_1 \land B_2 \land F\right) \lor
  \left(\lnot B_2 \land \xbefore{B_1}{F}\right)
\]
%
Thus, $System_{\mathrm{\algebra}}$ fails with $LP$ if:
\begin{itemize}
