...
%
The work~\cite{DM2012} was based on \ac{cspm} to allow an automatic analysis using the model checker \acsu{fdr}.
%
Thus, our strategy required the translation from \simulink to \ac{cspm}~\cite{JMS+2011}.
%
It then runs \acsu{fdr} to obtain several counter-examples (which are fault traces) ending in failures.
%
...
We only show eight counter-examples, but \acsu{fdr} generates a total of 64 counter-examples for this system.
...
%As we mentioned in~\cref{sec:faults-injection} we translated the monitor to \ac{cspm} according to our strategy and modified the observer to make \acsu{fdr} generate more counter-examples.
