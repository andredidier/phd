...
%TODO remove tracetoboolop
\def\tracetoboolop{\ensuremath\operatorname{\stackrel{\leadsto}{\text{\tiny B}}}}
\def\tracetofba{\ensuremath\operatorname{\stackrel{\leadsto}{\text{\tiny F}}}}
%TODO remove tracetoalgebraop
\def\tracetoalgebraop{\ensuremath\operatorname{\stackrel{\leadsto}{\text{\tiny XB}}}}
\newcommand{\tracetoalgebra}[1]{\ensuremath\langle{#1}\rangle_{XB}}
\newcommand{\tracetobool}[1]{\ensuremath\langle{#1}\rangle_{B}}
...
Joining the paths described in \cref{fig:strategy-overview-csp} and \cref{fig:strategy-overview-activation}, the complete proposed solution is summarized in \cref{fig:strategy-overview} (A and B, respectively).
%
\begin{figure}[htb]
  \centering
...
Using \cref{eq:general-pand-probability,eq:generators} we define the denotational probability\index{denotational probability} $\denotationalprobop$ of a list as:
%
\begin{subequations}
\begin{align}
\denotationalprob{xs} = &
  ...
  % - 
  %\sum_{ys \in R\left(S, xs\right)}\probability{ys}\label{eq:denotational-probability}\\
%%%%%%%%%%%%%
\generators_{xs} = & \generators\left(\True\right) - \listset{xs}\label{eq:generators-minus-list}
%%%%%
%R\left(S, xs\right) = & \listsof{S}{xs} - \left\{xs\right\}\nonumber
\end{align}
\end{subequations}
%
...
Finally, using \cref{eq:formula-probability}, and for generators $f_1$ and $f_2$, we demonstrate the equivalence to \cref{eq:or-probability} (the probability of \iac{OR} operator), calculated using the denotational semantics:
%
\begin{align}
S = & \vari{1} \lor \vari{2}\nonumber\\
%%%
...
All definitions in this \lcnamecref{sec:healthiness-conditions} refer to an algebra that has the following properties:
\begin{description}
...
\begin{alineasinline}
  ...
  \item modelling faults of the monitor using \ac{activation}, using expressions in Boolean Algebra\index{Boolean Algebra}, and
...
%
\begin{align*}
\text{\texttt{TRACE 1: }}&\tracetobool{\trace{s,b}} = S \land B \\
\text{\texttt{TRACE 2: }}&\tracetobool{\trace{b,s}} = B \land S \\
\text{\texttt{TRACE 3: }}&\tracetobool{\trace{a,b}} = A \land B \\
\text{\texttt{TRACE 4: }}&\tracetobool{\trace{b,a}} = B \land A \\
\text{\texttt{TRACE 5: }}&\tracetobool{\trace{a,s}} = A \land S \\
\text{\texttt{TRACE 6: }}&\tracetobool{\trace{a,s,b}} = A \land S \land B \\
\text{\texttt{TRACE 7: }}&\tracetobool{\trace{a,b,s}} = A \land B \land S \\
\text{\texttt{TRACE 8: }}&\tracetobool{\trace{b,a,s}} = B \land A \land S \\
\text{\texttt{TRACE 9: }}&\tracetobool{\trace{s,a,b}} = S \land A \land B \\
\text{\texttt{TRACE 10: }}&\tracetobool{\trace{s,b,a}} = S \land B \land A \\
\text{\texttt{TRACE 11: }}&\tracetobool{\trace{b,s,a}} = B \land S \land A 
\end{align*}
...
Applying mapping function \cref{eq:mapping-bool-set} for the previously shown set of traces, we obtain  following expression in \ac{algebra} (and in \ac{FBA}\index{Boolean Algebra!Free}):
\[
(A \land B) \lor (S \land (A \lor B))
\]
%
which is equivalent to \EMBRAER failure expression shown in \cref{tbl:acsAnnotations}.
%
...
%
\begin{align*}
\text{\texttt{TRACE 1: }}&\tracetoalgebra{\trace{s,b}} = \parsin{\xbefore{S}{B}}\\
\text{\texttt{TRACE 2: }}&\tracetoalgebra{\trace{b,s}} = \parsin{\xbefore{B}{S}}\\
\text{\texttt{TRACE 3: }}&\tracetoalgebra{\trace{a,b}} = \parsin{\xbefore{A}{B}}\\
\text{\texttt{TRACE 4: }}&\tracetoalgebra{\trace{b,a}} = \parsin{\xbefore{B}{A}}\\
\text{\texttt{TRACE 5: }}&\tracetoalgebra{\trace{a,s}} = \parsin{\xbefore{A}{S}}\\
\text{\texttt{TRACE 6: }}&\tracetoalgebra{\trace{a,s,b}} = 
  \xbefore{A}{\left(\xbefore{S}{B}\right)} \\
\text{\texttt{TRACE 7: }}&\tracetoalgebra{\trace{a,b,s}} = 
  \xbefore{A}{\left(\xbefore{B}{S}\right)} \\
\text{\texttt{TRACE 8: }}&\tracetoalgebra{\trace{b,a,s}} = 
  \xbefore{B}{\left(\xbefore{A}{S}\right)} \\
\text{\texttt{TRACE 9: }}&\tracetoalgebra{\trace{s,a,b}} = 
  \xbefore{S}{\left(\xbefore{A}{B}\right)} \\
\text{\texttt{TRACE 10: }}&\tracetoalgebra{\trace{s,b,a}} = 
  \xbefore{S}{\left(\xbefore{B}{A}\right)} \\
\text{\texttt{TRACE 11: }}&\tracetoalgebra{\trace{b,s,a}} = 
  \xbefore{B}{\left(\xbefore{S}{A}\right)} 
\end{align*}
...
\begin{align}
M_A = & 
  \left(\xbefore{S}{B} \land \lnot A\right) \lor
  \left(\xbefore{B}{S} \land \lnot A\right) \lor
  \left(\xbefore{A}{B} \land \lnot S\right) \lor \nonumber \\
  & \left(\xbefore{B}{A} \land \lnot S\right) \lor
  \left(\xbefore{A}{S} \land \lnot B\right) \lor
  \left(\xbefore{A}{\left(\xbefore{S}{B}\right)}\right) \lor \nonumber \\
  &\left(\xbefore{A}{\left(\xbefore{B}{S}\right)}\right) \lor
  \left(\xbefore{B}{\left(\xbefore{A}{S}\right)}\right) \lor
  \left(\xbefore{S}{\left(\xbefore{A}{B}\right)}\right) \lor \nonumber \\
  &\left(\xbefore{S}{\left(\xbefore{B}{A}\right)}\right) \lor
  \left(\xbefore{B}{\left(\xbefore{S}{A}\right)}\right) & \text{by \cref{eq:mapping-algebra-set}} \nonumber \\
%
  = & \left(S \land B \land \lnot A\right) \lor 
  \left(A \land B \land \lnot S\right) \lor
  \left(\xbefore{A}{S} \land \lnot B\right) \lor \nonumber \\
  & \left(\xbefore{A}{\left(\xbefore{S}{B}\right)}\right) \lor 
  \left(\xbefore{A}{\left(\xbefore{B}{S}\right)}\right) \lor
  \left(\xbefore{B}{\left(\xbefore{A}{S}\right)}\right) \lor \nonumber \\
  & \left(\xbefore{S}{\left(\xbefore{A}{B}\right)}\right) \lor 
  \left(\xbefore{S}{\left(\xbefore{B}{A}\right)}\right) \lor
  \left(\xbefore{B}{\left(\xbefore{S}{A}\right)}\right) 
  & \text{by \cref{thm:xbefore-inf-1}} \nonumber \\
%
  = & \left(S \land B \land \lnot A\right) \lor 
  \left(A \land B \land \lnot S\right) \lor
  \left(\xbefore{A}{S} \land \lnot B\right) \lor \nonumber \\
  & \left(B \land \left(\xbefore{A}{S}\right)\right) \lor
  \left(B \land \left(\xbefore{S}{A}\right)\right)
  & \text{by \cref{thm:and_xbefore_equiv_or_xbefore_expanded}} \nonumber \\
%
  = &\left(A \land B\right) \lor \left(B \land S\right) \lor
    \left(\xbefore{A}{S} \land \lnot B\right) 
\end{align}

The semantics of the above expression is:
\begin{alineasinline}
  \item fault $b$ ($\var{b}$) occurs and fault $a$ ($\var{a}$) or fault $s$ ($\var{s}$) occurs, or
  \item fault $a$ occurs before fault $s$ and fault $b$ does not occur, which is more precise than the expression found without considering order of events.
\end{alineasinline}
