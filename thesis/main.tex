...
Tradicionalmente, Árvores de Defeitos Estáticas são empregadas para este fim, mas Árvores de Defeitos Temporais e Dinâmicas têm ganhado evidência devido ao seu maior poder para modelar e detectar propagações complexas de defeitos que levam a uma falha.

Em um trabalho anterior, mostramos uma estratégia baseada na álgebra de processos CSP e modelos Simulink para obter rastros (sequências) de defeitos que levam a uma falha.
A partir dos rastros de defeitos nós descartamos a informação de ordenamento para obter expressões de estrutura  para Ávores de Defeitos Estáticas.
Ao contrário de descartar tal informação de ordenamento, poderíamos usá-la para obter expressões de estrutura para Árvores de Defeitos Temporais ou Dinâmicas.
No presente trabalho, apresentamos uma álgebra temporal de defeitos (com noção de propagação de defeitos) para analisar falhas em sistemas e provamos que ela é de fato uma álgebra Booleana.
...
%  \item uma álgebra temporal de defeitos (com noção de propagação de defeitos) para analisar falhas em sistema e provamos que ela é de fato uma álgebra Booleana.
%\end{alineasinline}
...
Com expressões na álgebra temporal de defeitos nós permitimos a verificação de propriedades de segurança (\emph{safety}) baseadas em Árvores de Defeitos Estáticas, Temporais ou Dinâmicas.
...
The laws of \ac{algebra} are given in a denotational semantics based in sets of lists of distinct elements.
...
\isabellehol{'s} theory files with all proofs are available at \url{http://www.cin.ufpe.br/~alrd/phd/phd-alrd-theory.zip}
...
We introduce dependability\index{Dependability} and faults modelling in \cref{sec:dependability}.
...
The work reported in~\cite[p. 289--302]{Sommerville2011} explain these properties---including dependability\index{Dependability}---with a focus in software.
...
The work reported in~\cite{ALR+2004} summarizes all concepts of and related to dependability\index{Dependability} for computing systems that contains software and hardware.
...
\end{alineasinline}
We use these methods to obtain an erroneous behaviour of the system which is compared to system's dependability attributes\index{Dependability!attributes of} (safety).
...
In the following we list all theorem and proofs concerning the laws presented in the \cref{chap:strategy}.
In this version we list the content of all theory files.
Each theorem, proof or corollary is followed by their own proof.

The theory about lists of distinct elements---or simply \emph{distinct lists}---is available in~\cite{HL2016} (we used the 2015 version the is available with Isabelle/HOL 2015).
