...
In important stages of critical systems, \acl{FTA} play essential roles.
At least three dependability means\footnote{The fourth dependability means is: fault removal. It is about detecting and removing failure from a real system, as in the development stage as in production stage.}\todo{Encontrar trabalho sobre meios de dependabilidade} can---or should---be achieved using \acp{FT}:
%
\begin{description}
  \item[Prevention] is about avoid incorporating faults during development. 
  A \ac{FTA} can indicate if the probability of failure of a subsystem is high, so it could be left to be incorporated when it becomes more reliable.
  \item[Forecasting] is about predicting likely faults so they can removed, or tackling their effects.
  After a system is in production, one could use its \acp{FT} to verify likely faults and evaluate its importance to take decisions.
  ...
  Also, the probability of replicated combinations can be evaluated, so a homogeneous replication could be replaced by a heterogeneous one, reducing the probability of failure\todo{Citar trabalho sobre redução de probabilidade com replicação heterogênea}.
\end{description}

\Ac{FTA} was introduced in~\cite{VGR+1981} with \ac{SFT}. 
\Iac{SFT} gives a general view---very often considering faults outside of the boundaries of a system---and represents, in a single view, different states in which a particular failure is active in a system.
