...
There is also \iac{WSp} that is slightly different to the \ac{CSp} gate.
...
\begin{description}
  ...
  Also, the probability of failure of replicated systems can be evaluated, so a homogeneous replication could be replaced by a heterogeneous one, reducing the probability of failure (in general)
  \todo{Cite work about failure probability reduction using heterogeneous replication}.
\end{description}
...
\begin{figure}[t]
  \centering
  \includegraphicsaspectratio[0.8]{dft-example-mrl2014}
...
They are still referred as \ac{BDD}, but the more spread version is the \ac{ROBDD}, which is an optimised version, based on Shannon's method (described in \FThandbook).

The general idea to create a graph is to assume a value to each variable and evaluate the other variables in both cases of the assumed values.
This exactly what Shannon's method defines.
One implementation of the \ac{BDD} creation algorithm is based on a set of true and false value sets, denoted by the environment of the create algorithm.
For example, given the expression
\todo{talk about evaluation: the probability is calculated for all paths ending in ``1'' }
...
\begin{figure}
  \centering
  \includegraphicsaspectratio[0.6]{simple-dependency-tree}
