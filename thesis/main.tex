...
In this section we show how to formalize two kinds of acceptance criteria of \acp{FT}: (i) \ac{MCSeq}, the quantity of faults in the minimal sequences that cause the root failure, and (ii) root probability, provided the probability of sequences occurrences.
These criteria are representative and other criteria can be modelled similarly.

Using the denotational semantics of \ac{algebra}, the \ac{MCSeq} are the lists with least length, because all list have no repeated elements and each list represent a possible combination the leads to a failure.

\begin{definition}[Minimal cut sequences]
Let $F$ be a formula in \ac{algebra} and $L$ the denotational semantics of $F$ (a set of \distinctlists), the minimal cut sequences $\mcseqop$ of $F$ is:
%
\begin{equation}
...
The work reported in~\cite{FAR1976} shows how to calculate the probability of \iac{PAND} gate:
\begin{align}
P\left(t\right) & = \probabilityt{T_1 \le T_2 }\nonumber\\
& = \int_0^t P'_1(x) P_2(x) dx\label{eq:pand-probability}
\end{align}
%
where $P_i$ is the probability of the occurrence of the $i$th fault (input), and $T_i$ is the time that the $i$th fault occurs.

Each list in the denotational semantics of \ac{algebra} is equivalent to \iac{PAND} gate in \cref{eq:pand-probability}.
For example, given the formula $\xbefore{\left(\xbefore{\var{f_1}}{\var{f_2}}\right)}{\var{f_3}}$ (with all $f_i$ independent and distinct), its denotational semantics is $\left\{\left[f_1,f_2,f_3\right]\right\}$, hence, considering $T_i$ the time that $f_i$ occurs, the probability of the formula is is given by the probability of the list $\left[f_1,f_2,f_3\right]$:
\begin{align*}
\probabilityt{\left[f_1,f_2,f_3\right]} = &\probabilityt{\left[f_1,f_2\right]} \times \probability{\left[f_2,f_3\right]}\\
=& \left(\probabilityt{T_1 < T_2}\right) \times \left(\probabilityt{T_2 < T_3}\right)
\end{align*}

The $\le$ can be safely replaced by $<$ in the formula because $T_1 = T_2$ if, and only if, the faults are the same or are dependent.

The formula $\left(\xbefore{\var{f_1}}{\var{f_2}}\right) \lor \left(\xbefore{\var{f_2}}{\var{f_1}}\right)$ (considering only the two generators ($f_1$ and $f_2$), has denotational semantics $\left\{\left[f_1, f_2\right], \left[f_2, f_1\right]\right\}$ and the probability of the formula is:
%
\begin{align}
\probabilityt{\left\{\left[f_1, f_2\right], \left[f_2, f_1\right]\right\}} = & \probabilityt{\left[f_1, f_2\right]} + \probabilityt{\left[f_2, f_1\right]} \nonumber\\
& - \probabilityt{\left[f_1, f_2\right]} \times \probabilityt{\left[f_2, f_1\right]}\nonumber\\
= & \probabilityt{\left[f_1, f_2\right]} + \probabilityt{\left[f_2, f_1\right]}\nonumber\\
= & \probabilityt{T_1 < T_2} + \probabilityt{T_2 < T_1}\label{eq:prob-xbefore-sup-equiv-inf}
\end{align}
%
The term $\probabilityt{\left[f_1, f_2\right]} \times \probabilityt{\left[f_2, f_1\right]}$ can be safely removed because both situations cannot occur simultaneously.

The probability of a formula like $\left(\xbefore{\var{f_1}}{\var{f_2}}\right) \lor \left(\xbefore{\var{f_2}}{\var{f_1}}\right)$ must be equal to the probability of a formula $\var{f_1} \land \var{f_2}$, as a these formulas are equivalent accordingly to \cref{thm:xbefore-sup-equiv-inf}.

\begin{lemma}
Let $f_1$ and $f_2$ be distinct faults and independent events, then
\[
\probabilityt{\left(\xbefore{\var{f_1}}{\var{f_2}}\right) \lor \left(\xbefore{\var{f_2}}{\var{f_1}}\right)} = \probabilityt{\var{f_1} \land \var{f_2}}
\]
\end{lemma}
\begin{proof}
Let $P_i$ be the probability of occurrence of fault $i$ before $t$ ($P_i(t) = \probabilityt{T_i}$), and $T_i$ the time fault $i$ occurs.
From \cref{eq:prob-xbefore-sup-equiv-inf}
\begin{align*}
\probabilityt{T_1 < T_2} + \probabilityt{T_2 < T_1} = &
\int_0^t P_1'(x) P_2(x) dx +
\int_0^t P_2'(x) P_1(x) dx \\
%
= & \int_0^t \left(P_1'(x) P_2(x) + P_2'(x) P_1(x)\right) dx\\
= & \int_0^t \left(P_1(x) P_2(x)\right)' dx\\
= & P_1(t) P_2(t)
\end{align*}
\end{proof}

\begin{definition}[Probability of a formula in \ac{algebra}]
Let $F$ be a formula in \ac{algebra}, and $L$ its denotational semantics (a set of \distinctlists), then:
%
\begin{equation}
\probabilityt{F} = \sum_{xs \in L}\probabilityt{xs}
\end{equation}
\end{definition}
...
For example, the $\var{f_1} \land \var{f_2} \land \lnot \var{f_3}$ has probability: $\probabilityt{\left[f_1,f_2\right]} + \probabilityt{\left[f_2,f_1\right]}$.
