...
\begin{enumerate}
  ...
  \item Illustrate the application of the laws on \hl{two}\adnote{Pensei em usar mais estudos de caso, se der tempo, claro.} real case studies, provided by our industrial partner.
\end{enumerate}
...
The concepts of dependability\index{dependability} are: 
\begin{alineasinline}
  \item threats to, 
...
The analysis of \acp{TFT} uses a denotational semantics based on \emph{sequence values}\index{Temporal Fault Tree!sequence value} to express ordering of events, thus tackling \acg{PAND} order.
...
They are focused on directly expressing ordering relationships rather than different states of a system. Basically they extend \acg{SFT} \ac{PAND} gates, allowing analysis of \ac{FT} with such gates.
...
For \acp{TFT}, in~\cite{Walker2009} \acp{MCSeq} are represented as \iac{DNF} using \ac{AND} and the temporal operators (\ac{PAND}, \ac{POR}, and \ac{SAND}) as doublets (a single temporal relation)---which are the minimal terms---or prime implicants---in the \ac{DNF}.
In a doublet, the expression is a product (of \ac{AND}) of temporal operators.

The normal form for \ac{TFT} is similar to \ac{SFT}: it is \iac{DNF} with temporal operators (\ac{PAND}, \ac{POR}, \ac{SAND}) in the minterms.
...
\begin{table}
  ...
  %
  \hline\noalign{\smallskip}
  \ac{PAND} & $B \land \parsin{A \ibefore B}$ &\\
  %
  \hline\noalign{\smallskip}
  \end{tabular*}
\end{table}
...
Instead of tackling basic events ordering (as in \ac{PAND}), it consider a causal relation over a gate, as for example, a relation of a basic event and a higher-level intermediary event.
...
\begin{alineasinline}
  ...
  \item expand the paths based on Shannon's method (described in \FThandbook).
\end{alineasinline}
...
Although the \FThandbook introduces several gates, the vast majority of \ac{SFT} analysis would fit in \acp{FT} with only \ac{AND} and \ac{OR} gates (coherent \acp{FT})\index{Fault tree!coherent}.
...
\begin{alineasinline}
  \item the second example presented in~\cite{Oliv2006}, which highlights the problem when using NOT gates (\cref{sec:non-coherent-misleads-example}), and 
  \item the second example presented in~\cite{Andrews2001}, which defends the usefulness of NOT gates in a multitasking system (\cref{sec:non-coherent-usefulness}).
\end{alineasinline}
...
\]
%
which is equivalent to the set of sets above and is equivalent to \EMBRAER failure logic expression shown in \cref{tbl:acsAnnotations} (with \ac{AND} gates as $\inter$ and \ac{OR} gates as $\union$).
%
...
In \cref{fig:status-of-thesis} we show what: 
\begin{alineasinline}
  \item was done in previous work and is used as input in this thesis, 
  \item is already done in this thesis, 
  \item will be done in the next months (see \cref{tbl:tasks-schedule} for tasks schedule), and 
  \item will be done in future work, after the thesis' defence.
\end{alineasinline}
...
\IBGEtab{%
  ...
  \midrule
    Prove soundness and completeness theorems for the DNF of \ac{algebra} & 20/07/2016 & 75 & 107 \\
  \midrule
...
The next steps in this thesis are to conclude the work-in-progress of the ``Faults Modelling'' and ``Automatic FTA'' blocks in \cref{fig:status-of-thesis}.
An initial version of the theory of the \ac{activation} is done, but we need to adapt the failure predicates to \theac{algebra}.
...
Although we do not use negation (\ac{NOT} operator) with XBefore in our case study, it is part of \ac{algebra}, so it could be used.
...
Considering that \acp{DFT} were conceived as a visual representation of \aca{DTMC}, then we may say that \acp{DFT} can be used to represent several states, thus they are suitable to propose the architectural model modifications as shown in \cref{fig:strategy-overview,fig:status-of-thesis}.
The definition of ``Faults Modelling and Fault Tolerance Patterns'' and the automatic proposal of ``Architectural Model Modifications'' are left as future work.
