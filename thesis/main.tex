...
\begin{definition}[Minimal cut sequence]
Given ...
%TODO
\todo{Continuar daqui}
\end{definition}
...
Following the same idea with denotational semantics, we can calculate root probability from a formula in \ac{algebra}.
...
\probabilityt{\lnot a} &= 1 - \probabilityt{a}
\end{align*}
%
...
The probability of a component be in an operational state before the failure of another is \emph{approximately} the probability of a failure in the second.
Similarly, the probability of a component fail before the operational state of another is \emph{approximately} the probability of the failure of the first.
