...
The valves closes a gas flow if the sensor detects a leak.
If the valve is closed for a certain amount of time, the pressure on the system may increase, and then, a relief valve diverts the gas flow elsewhere.
...
\begin{alineasinline}
  ...
  \item obtaining failure probability from a formula with explicit \ac{NOT} operators.
\end{alineasinline}
%
...
\label{sec:top-event-probabihlity-explicit-not}

The work reported in~\cite{Andrews2001} shows a case-study with an explicit \ac{NOT} operator.
It is a leak protection system 
