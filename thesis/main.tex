...
The general case (reported in~\cite{FAR1976}) of \iac{PAND} gate with $n$ inputs is the probability of the sequence of faults $f_i$, $i \in \left\{~1, \ldots, n~\right\}$, where $f_n$ is the first fault, and $f_1$ is the last one:
%
\begin{align}
...
\end{align}
%
where $P_i$ is the probability of the occurrence of fault $f_i$.
...
For example, $\probability{\left[f_1,f_2\right]}$ contains the situations that $f_3$ does not occur, or occurs in some time: $\probability{\left[f_1,f_2,f_3\right]}$, $\probability{\left[f_1,f_3,f_2\right]}$, and $\probability{\left[f_3,f_1,f_2\right]}$.
...
Using \cref{eq:listsof,eq:generators} we define the denotational probability\index{denotational probability} $\denotationalprobop$ of a list as:
\begin{align}
\denotationalprob{S}{xs} = &
  \probability{xs} - 
...
\end{align}
%
%For example, to calculate $\denotationalprob{S}{\left[f_1, f_2\right]}$ for set of generators $\left\{f_1,f_2,f_3\right\}$, we have:
%\begin{align*}
%\denotationalprob{S}{\left[f_1,f_2\right]} = & \probability{\left[f_1,f_2\right]}
...
Let $F$ be a formula in \ac{algebra}, then:
%
\begin{equation}
\label{eq:algebra-probability}
\probability{F} = \sum_{xs \in \denote{F}}\denotationalprob{S}{xs}
\end{equation}
\end{definition}
...
%
\begin{align}
\probability{\lnot \var{f_1}} = & \denotationalprob{\lnot \var{f_1}}{\left[\right]} + \denotationalprob{\lnot \var{f_1}}{\left[f_2\right]} & \text{by \cref{eq:algebra-probability}} \nonumber\\
%%%%%%%%%%%%%
...
%
\begin{align}
\denotationalprobop_L&\left\{\left(\xbefore{\var{f_1}}{\var{f_2}}\right) \lor \left(\xbefore{\var{f_2}}{\var{f_1}}\right)\right\}\nonumber\\
%%%%
=&\denotationalprob{S}{\left[f_1,f_2\right]} + \denotationalprob{S}{\left[f_2,f_1\right]} & \text{by \cref{eq:algebra-probability}} \nonumber\\
%%%%
...
\probability{S} &= 
\denotationalprob{S}{\left[f_1,f_2\right]} + \denotationalprob{S}{\left[f_2,f_1\right]} \nonumber\\
%%%%%%%%
...
S &= \vari{1} \lor \vari{2}\nonumber\\
%%%
\probability{S} = & \denotationalprob{S}{\left[f_1\right]} +
  \denotationalprob{S}{\left[f_2\right]} +
  \denotationalprob{S}{\left[f_1,f_2\right]} +
  \denotationalprob{S}{\left[f_2,f_1\right]}
\nonumber\\
%%%%%%%%%%%
