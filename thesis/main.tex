...
The \emph{function} of a system is what the system is intended to do, and its \emph{behaviour} is what the system does to implement the function.
...
The \emph{service} delivered by a system is its behaviour as it is perceived by its users.
A system can both provider and consumer of other services.
...
This concept of compositionality in systems, is what originated the concept of \ac{SoS}.
...
A failure is a service deviation perceived on systems' boundaries.
An error is the part of the total state of a system that leads to subsequent service failure.
Depending on how a system tolerate internal errors, many errors may not reach system's boundaries.
...
One is more general, difficult to measure: ``the ability to deliver service that can be justifiably be \emph{trusted}''.
...
The \emph{Attributes of dependability}\index{dependability!attributes of} are:
\begin{description}
...
\emph{Means to attain dependability}\index{dependability!means to attain}\ldots\todo{Write about means to attain dependability}
...
In this work we focus on some aspects of dependability\index{dependability}.
\ldots\todo{Elaborate the focus of the aspects of dependability}
...
\Acpcite{TFT} were created aiming at fully implement the \FThandbook~\cite{VGR+1981}.
The \pandtext gate was first defined for \acp{SFT}, but its analysis was left open in the handbook.
\Acp{TFT} create a new kind of structure expressions, using \emph{sequence values}\index{Temporal Fault Tree!sequence value} to express ordering of events, thus tackling \pandtext's order.
...
\begin{subequations}
\begin{align}
  \parsin{X \pand Y} \lor \parsin{X \sand Y} \lor \parsin{Y \pand X} &= X \land Y & \text{Conjunctive Completion Law}\\
