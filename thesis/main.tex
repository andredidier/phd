...
One important concern on employing \ac{FTA} is whether a \ac{FT} indeed represents a system behaviour.
...
The reduction of structure expressions in \ac{TFT} is based in a combination of: (i) algebraic reduction---that can lead to an ever-growing rules application---, (ii) modularisation of independent subtrees, and (iii) \ac{TDT}---which are limited to seven basic events, due to exponential grow of the tree.

\begin{sloppypar}
Most of the mathematical models\adnote{Cite all works that models using DTMC/CTMC} for \ac{DFT} are based on the formalisation of \ac{DTMC} or \ac{CTMC}, because \acp{DFT} were initially conceived to be a visual representation of such models.
As both \ac{DTMC} and \ac{CTMC} are state-based, they suffer the state-space explosion problem\adnote{Cite works about state-explosion: \url{http://ieeexplore.ieee.org/xpls/abs\_all.jsp?arnumber=1205180}, \url{http://www.sciencedirect.com/science/article/pii/S156783260200067X}}.
The work reported in~\cite{SAE1996b} shows some techniques to overcome this problem, but the reduction depends on systems' models, whether they are independent or not, which can be impracticable.
\end{sloppypar}
...
Is there a mathematical model that is set-based and similar to \ac{FBA} that is:
\begin{alineas}
  \item capable of representing events ordering of \ac{TFT} and \ac{DFT}\label{question:ordering-representation};
  \item allows both qualitative and quantitative analyses of both \ac{TFT} and \ac{DFT}\label{question:analyses};
...
In this work we present a denotational semantics and an associated algebra, called \ac{algebra}, to analyse ordering of fault events.
Fault events are either extracted from a nominal model with injected faults as in~\cite{DM2012,Didier2012}, or modelled using a proposed notation, called \ac{activation}.
...
Structure expressions\index{structure expression} are written with an algebra that has operators \ac{OR} and \ac{AND}, and three new operators to express events ordering: (i) \ac{NIBefore}, (ii) \ac{SIMLT}, and (iii) \ac{IBefore}.
