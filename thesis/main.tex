...
To analyse fault trees, their structures are then abstracted as Boolean expressions called \emph{structure expressions}\index{structure expression}.
The analysis with these two traditional gates uses a well-defined algorithm based on the Shannon's method---which originated the \ac{BDD}\index{Binary Decision Diagrams}---to obtain minimal cut sets\index{Minimal Cut!Sets} from the structure expressions\index{structure expression} and a general formula to calculate the probability of top events.
...
We considered faults in components or subsystems, but if we obtain failure expressions of a whole system, they are in fact structure expressions\index{structure expression} of a fault tree.
%
...
A predicate in \ac{activation} generates an expression in \theac{algebra}.
...
Structure expressions\index{structure expression} are used to analyse fault trees.
...
In \cref{sec:structure-expressions} we explain these structure expressions\index{structure expression} for \acp{SFT}, \acp{TFT} and \acp{DFT}.
...
The \FThandbook shows an algorithm based on Shannon's method to reduce structure expressions\index{structure expression} to obtain minimal cut sets\index{Minimal Cut!Sets}.
