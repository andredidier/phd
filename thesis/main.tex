...
Following the use of the denotational semantics, we can calculate the root probability from a formula in \ac{algebra}.
...
The repeated situations must be removed.
...
For example, $\filter{\left[f_1,f_2\right]}{\left[f_1, f_2, f_3\right]} = \left[f_1,f_2\right]$.
\end{sloppypar}
...
The probability in a formula in \ac{algebra} is calculated using its denotational semantics as \acp{OR} of \acp{XBefore}.
For example, the formula $\left(\xbefore{\var{f_1}}{\var{f_2}}\right) \lor \left(\xbefore{\var{f_2}}{\var{f_1}}\right)$, considering only the two generators ($f_1$ and $f_2$), has denotational semantics $\left\{\left[f_1, f_2\right], \left[f_2, f_1\right]\right\}$ and the probability of the formula is:
%
\begin{align}
...
For example, the formula $\var{f_1} \land \var{f_2} \land \lnot \var{f_3}$ has probability $P_1(t) \times P_2(t)$.
