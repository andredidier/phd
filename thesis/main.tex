...
Each event in the traces becomes a variable, and each list becomes \iac{XBefore} with the \acp{AND} of the variables of the remaining generators.
%
\begin{subequations}
\begin{align}
\tracetoalgebra{\trace{}} = & \top\\
%
\tracetoalgebra{\trace{f}} = & \var{f}\\
%
...
\tracetoalgebra{\left\{ tr_1, tr_2, \ldots, tr_n  \right\}} =& 
  ...
  \lnot \bigvee_{j ~\in~ \generators_{tr_i}} \var{f_j}\right)
\end{align}
\end{subequations}
%
...
Now, by using \ac{algebra} with the \ac{XBefore} operator and a mapping function ($\tracetoalgebraop$), we can capture each possible individual sequences as generated by the work~\cite{DM2012}:
%
\begin{align*}
\text{\texttt{TRACE 1: }}&[s,b] \tracetoalgebraop \parsin{\xbefore{S}{B}} \inter -A & \left\{[s,b]\right\}\\
\text{\texttt{TRACE 2: }}&[b,s] \tracetoalgebraop \parsin{\xbefore{B}{S}} \inter -A & \left\{[b,s]\right\}\\
\text{\texttt{TRACE 3: }}&[a,b] \tracetoalgebraop \parsin{\xbefore{A}{B}} \inter -S & \left\{[a,b]\right\}\\
\text{\texttt{TRACE 4: }}&[b,a] \tracetoalgebraop \parsin{\xbefore{B}{A}} \inter -S & \left\{[b,a]\right\}\\
\text{\texttt{TRACE 5: }}&[a,s] \tracetoalgebraop \parsin{\xbefore{A}{S}} \inter -B & \left\{[a,s]\right\}\\
\text{\texttt{TRACE 6: }}&[a,s,b] \tracetoalgebraop \xbefore{\xbefore{A}{S}}{B} & \left\{[a,s,b]\right\}\\
\text{\texttt{TRACE 7: }}&[a,b,s] \tracetoalgebraop \xbefore{\xbefore{A}{B}}{S} & \left\{[a,b,s]\right\}\\
\text{\texttt{TRACE 8: }}&[b,a,s] \tracetoalgebraop \xbefore{\xbefore{B}{A}}{S} & \left\{[b,a,s]\right\}
\end{align*}
