...
\newcommand{\denotationalprob}[2]{\ensuremath \denotationalprobop_{#1}\left\{#2\right\}}
...
%TODO acm: Aqui eu pegaria o gancho da AADL (assertion framework)...
...
\Theac{activation} allows reasoning about faults that are not modelled in \simulink as, for example, common cause or environmental faults.
...
\begin{description}
  ...
  Timing in this gate between trigger event and dependent events occurrences can be instantaneous (like in \acg{TFT} \ac{SAND} gate), or a small amount of time, thus implying an order of occurrence, depending on the kind of dependency.
  \item[\Ac{CSp}:]
...
This definition represents a Boolean\index{Boolean Algebra} algebra as an algebra of sets and does not rely on Boolean axioms (which can be misleading, case there is an unfounded axiom).
...
We showed in \cref{sec:fault-trees} that there is a consistent presence of order-based operators to analyse \acp{TFT} and \acp{DFT}, and that each approach describes a new algebra based on different representations of events ordering with similar theorems to reduce expressions to a canonical form.

From the need to tackle events ordering and from the ordering information we had from fault injection that we developed in~\cite{DM2012}, we defined a list-based algebra, called \acf{algebra}, to express and analyse systems considering events ordering.
...
These lists are composed by non-repeated elements (\distinctlists) because the events in \aclp{FT} are non-repairable, thus they do not occur more than once.
...
They are abbreviations that some operators satisfy altogether\todo{Verificar significado de altogether} or individually.
...
\section{Qualitative and quantitative properties}
\label{sec:qualitative-quantitative-properties}

In \cref{sec:fault-trees} we showed the kind of results that are obtained in \ac{FTA}.
In this section we show how to formalize two kinds of properties of \acp{FT}: (i) \ac{MCSeq}, the quantity of faults in the minimal sequences that cause the root failure, and (ii) root probability, provided the probability of sequences occurrences.
These properties are representative and other criteria can be modelled similarly.

The denotational semantics of \ac{algebra} is a set of \distinctlists, thus each list has no repeated elements and each list represent a possible combination of faults that causes the root failure.
\Acp{MCSeq} are the lists with least length, thus have minimal lengths of the lists.

\begin{definition}[Minimal cut sequences]
Let $F$ be a formula in \ac{algebra} and $L$ the denotational semantics of $F$, the minimal cut sequences $\mcseqop$ of $F$ are:
%
\begin{equation}
\mcseq{F} = \left\{ xs \middle| xs \in L \bullet \length{xs} = minL \right\}
\end{equation}
%
where
\[
minL = \Min{\left\{ \length{xs} \middle | xs \in L\right\}}
\]
%
and $\Minop$ returns the smallest number in the given set.
\end{definition}

The work reported in~\cite{Merle2010} shows how to calculate the probability of \iac{PAND} gate:
\begin{align}
P\left(t\right) & = \probabilityt{T_1 \le T_2 }\nonumber\\
& = \int_0^t P'_2(x) P_1(x) dx\label{eq:pand-probability}
\end{align}
%
where $P_i$ is the probability of the occurrence of the $i$th fault (cumulative distribution function), and $T_i$ is the time that the $i$th fault occurs.
The general case (reported in~\cite{FAR1976}) of a \ac{PAND} gate with $n$ inputs is:
%
\begin{align}
\probability{\left[f_n,f_{n-1}\ldots,f_2,f_1\right]} = &\int_0^t\fiti{1}\int_0^{t_1}\fiti{2}\ldots\int_0^{t_{n-2}}\fiti{n-1}\nonumber\\
& \int_0^{t_{n-1}}\fiti{n} dt_n dt_{n-1}\ldots dt_2 dt_1
\end{align}
%
...
For example, $\probability{\left[f_1,f_2\right]}$ contains the situations of $\probability{\left[f_1,f_2,f_3\right]}$.
To isolate these cases, we use the filter notation, grouping all lists that contains repeated situations to calculate probability.
The probability of a list $xs$ in the denotational semantics of \ac{algebra} $L$ is:
\begin{align}
\denotationalprob{L}{xs} = \probability{xs} - 
\sum_{\left\{ ys | ys \in L - \left\{xs\right\} \bullet xs = \filter{xs}{ys}\right\}} \probability{ys}\label{eq:denotational-probability}
\end{align}
%
where $\filter{xs}{ys}$ removes all elements from $ys$ that are not in $xs$.
For example, $\filter{\left[f_1,f_2\right]}{\left[f_1, f_3, f_2\right]} = \left[f_1,f_2\right]$.
\end{sloppypar}

We use the traditional probabilities calculations~\cite{Merle2010} as reference, and to calculate probabilities of formulas in \ac{algebra}.
\begin{subequations}
\begin{align}
\probability{\vari{1} \land \vari{2}} = & \Fit{1} \times \Fit{2}\label{eq:and-probability}\\
\probability{\vari{1} \lor \vari{2}} = & \Fit{1} + \Fit{2} - \Fit{1} \times \Fit{2}
\end{align}
\end{subequations}

The probability in a formula in \ac{algebra} is calculated using its denotational semantics.
...
%
\begin{align}
\denotationalprob{L}{\left[f_1, f_2\right], \left[f_2, f_1\right]} =&
\denotationalprob{L}{\left[f_1,f_2\right]} + \denotationalprob{L}{\left[f_2,f_1\right]} - \nonumber\\
&\denotationalprob{L}{\left[f_1,f_2\right]} \times \denotationalprob{L}{\left[f_2,f_1\right]} \nonumber\\
%%%%
=&\denotationalprob{L}{\left[f_1,f_2\right]} + \denotationalprob{L}{\left[f_2,f_1\right]} & \text{ignored $\times$} \nonumber\\
%%%%
...
\end{align}
%
The term $\denotationalprob{L}{\left[f_1, f_2\right]} \times \denotationalprob{L}{\left[f_2, f_1\right]}$ can be safely removed because both situations cannot occur simultaneously.
...
Let $F$ be a formula in \ac{algebra}, and $L$ its denotational semantics, then:
%
\begin{equation}
\label{eq:algebra-probability}
\probability{F} = \sum_{xs \in L}\denotationalprob{L}{xs}
\end{equation}
\end{definition}
...
It means that even if a formula contains \iac{NOT} operator, we still safely obtain a probability value, without concerning about complement probabilities as tackled in~\cite{Andrews2001}.
For example, the probability of $\lnot \var{f_1}$ (for generators $f_1$ and $f_2$) is:
%
\begin{align}
\probability{\lnot \var{f_1}} = & \denotationalprob{L}{\left[\right],\left[f_2\right]} \nonumber\\
%%%%%%%%%%%%%
= &\denotationalprob{L}{\left[\right]} + \denotationalprob{L}{\left[f_2\right]} & \text{by \cref{eq:algebra-probability}} \nonumber\\
%%%%%%%%%%%%%
= &\probability{\left[\right]} - \probability{\left[f_2\right]}
+ \probability{\left[f_2\right]} & \text{by \cref{eq:denotational-probability}} \nonumber\\
%%%%%%%%%%%%%
...
The empty list is the special case to represent the approximation of the probability calculation.
When the empty list appears in a denotational semantics it works as the universal complement probability of any other list.

Another example including complement is the formula $\var{f_1} \land \var{f_2} \land \lnot \var{f_3}$, which has probability $P_1(t) \times P_2(t)$:
%
\begin{align}
\probability{\var{f_1} \land \var{f_2} \land \lnot \var{f_3}} &= 
\denotationalprob{L}{\left[f_1,f_2\right], \left[f_2,f_1\right]} \nonumber\\
&= P_1(t) \times P_2(t) & \text{by \cref{eq:prob-xbefore-sup-equiv-inf}}
\end{align}

Finally, using \cref{eq:algebra-probability}, we demonstrate the equivalence to probability of \iac{OR} operator, calculated using the denotational semantics:
%
\begin{align}
\probability{\vari{1} \lor \vari{2}} = & \denotationalprob{L}{
  \left[f_1\right],
  \left[f_2\right],
  \left[f_1,f_2\right],
  \left[f_2,f_1\right]
...
%%%%%%%%%%%
= &\probability{\left[f_1\right]} - \probability{\left[f_1,f_2\right]} - \probability{\left[f_2,f_1\right]} +\nonumber\\
&\probability{\left[f_2\right]} - \probability{\left[f_1,f_2\right]} - \probability{\left[f_2,f_1\right]} +\nonumber\\
&\probability{\left[f_1,f_2\right]} + \probability{\left[f_2,f_1\right]}\nonumber\\
%%%%%%%%%%%
= &\probability{\left[f_1\right]} + \probability{\left[f_2\right]} - \left(\probability{\left[f_1,f_2\right]} + \probability{\left[f_2,f_1\right]}\right)\nonumber\\
%%%%%%%%%%%
...
The acceptance criteria for a structure expression (written as a formula $F$) is given in terms of the minimal cut sequences length.

\begin{definition}[Minimal cut sequences length]
Let $F$ be a formula in \ac{algebra} and $L$ the denotational semantics of $F$, the qualitative acceptance criteria of the minimal cut sequences length be less than or equal to $n$ is:

\begin{equation}
\lengthacceptance{F}{n} = \Min{\left\{ \length{xs} \middle | xs \in L\right\}} \le n
\end{equation}
\end{definition}


%%%%%%%%%%%%%%%%%%Apagar daqui em diante.
In \ac{algebra}, we abstract the probability calculation, assuming that the probability of all \ac{XBefore} relations and the probability of individual faults are given.
For example, if a system have three faults $a$, $b$, and $c$, and a failure formula is given by $\left(a \land c\right) \lor \left(\xbefore{a}{b}\right)$, then the needed probabilities to calculate $\probabilityt{\left(a \land c\right) \lor \left(\xbefore{a}{b}\right)}$ are $\probabilityt{\xbefore{a}{b}}$, $\probabilityt{a}$ and $\probabilityt{c}$.
The probability of $\probabilityt{\xbefore{a}{b}}$ can be calculated as shown in \cref{eq:pand-probability}, assuming $T_1$ the time fault $a$ occurs, and $T_2$ the time $b$ occurs.
The probabilities for other operators, are given as usual:
\begin{subequations}
\begin{align}
\probabilityt{a \land b} &= \probabilityt{a} \times \probabilityt{b}\\
\probabilityt{a \lor b} &= \probabilityt{a} + \probabilityt{b} -\probabilityt{a} \times \probabilityt{b}\\
\probabilityt{\lnot a} &= 1 - \probabilityt{a} \approx 1
\end{align}
\end{subequations}
%
The \ac{NOT} operator is a special case in the probability analysis.
When analysing \acp{FT}, the probability of a component be functional is near $1$, which causes the probability to be ignored in general.
The \ac{NOT} operator when combined with any other (\ac{AND}, \ac{OR}, and \ac{XBefore}) results on a nearly safe approximation to $1$, for variables $a$ and $b$:
\begin{subequations}
\begin{align}
\probabilityt{\lnot a \land b} &= \probabilityt{\lnot a} \times \probabilityt{b} \approx \probabilityt{b}\\
\probabilityt{\lnot a \lor b}  &= \probabilityt{\lnot a} + \probabilityt{b} - \probabilityt{\lnot a} \times \probabilityt{b} \approx 1\\
\probabilityt{\xbefore{\lnot a}{b}} &\approx \probabilityt{b} ~\text{(see \cref{eq:pand-probability})}\label{eq:prob-not-xbefore}\\
\probabilityt{\xbefore{a}{\lnot b}} &\approx \probabilityt{a} ~\text{(see \cref{eq:pand-probability})}\label{eq:prob-xbefore-not}
\end{align}
\end{subequations}

\Cref{eq:prob-not-xbefore,eq:prob-xbefore-not} follow the same approximation shown in the work reported in~\cite{Andrews2001}.
The probability of an operational state before a failure is \emph{approximately} the probability of the failure---\cref{eq:prob-not-xbefore}.
Similarly, the probability of a failure before an operational state is \emph{approximately} the probability of the failure---\cref{eq:prob-xbefore-not}.

\begin{definition}[Root probability]
Given...
%TODO
\todo{Continuar daqui}
\end{definition}
...
\asnote{A definição de completess é intuitiva, mas a de soundness, não. Esperaria que fosse algo como a equivalência de duas formulas estivesse relacionada à equivalência das duas sequencias que representassem estas formulas. Da forma como está, uma mesma formula pode ser equivalente a duas sequencias diferentes, o que geraria uma inconsistência. }
...
\asnote{P não deveria estar quantificado universalmente no primeiro predicado? E no segundo, a quantificação é existencial mesmo?
A noção de soundness, como no caso anterior, continua parecendo muito fraca. }
...
\asnote{Vocês confirmaram com a Embraer que isso pode ser divulgado mesmo? No caso de Gustavo, mesmo exemplos que eles descaracterizam, não permitem que apresentemos assim de forma explícita a modelagem. Nós temos apresentado apenas os resultados. }
\asnote{Ficou esquisito não utilizar o not no estudo de caso, após tanta ênfase dada ao operador. Poderia tentar estender o estudo de caso de forma a usar o not.}
