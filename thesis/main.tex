...
\newlist{rqenum}{enumerate}{1} % also creates a counter called 'propenumi'
\setlist[rqenum]{label=\alph*), ref=\alph*)}
%\crefalias{rqenumi}{researchquestion} 
...
The \acp{MCS} are obtained by reducing structure expressions to a normal form, in which each term is a combination of variables (basic events) with \ac{AND} gates, and the terms are combined as \ac{OR} gates.
These minimal terms are also called \emph{prime implicants} or \emph{minterms}.
The Shannon's method originated a formalism to reduce structure expressions called \ac{BDD}\index{Binary Decision Diagrams}.
...
The algebra created in this work defines the NOT operator and allows its use\amnote{without introducing incoerence?} \adnote{Prefiro deixar assim mesmo...}, as we show in \cref{sec:strategy}.
\end{sloppypar}
...
%
Our strategy throws away the ordering information of the fault events sequences to generate failure expressions for components or subsystems for \acp{SFT}.
%
...
\begin{alineasinline}
  ...
  \item \ac{TDT}---which are limited to seven basic events, due to exponential growth.
\end{alineasinline}
...
The works reported in~\cite{BKK+2003,BHH+2003,SAE1996b} show techniques to overcome this problem, but the reduction can be infeasible because it depends on systems' models, whether they are independent or not.
\end{sloppypar}
...
Although there is a tool to model and analyse systems using \ac{hiphops}, \acp{FT} construction is based on an algorithm, without proofs for soundness nor completeness.
\end{sloppypar}
...
``Is there a mathematical model that is set-based and similar to \ac{FBA}?''.
...
\begin{rqenum}
  ...
  \item perform reduction of structure expressions to a normal form at least as efficient as current approaches\label{question:efficiency}?
\end{rqenum}
%
In this version of the thesis we propose the mathematical model that is set-based and is similar to \ac{FBA}, and answer \cref{question:ordering-representation}.
We open the discussion to answer \cref{question:gap}, but \cref{question:analyses,question:efficiency} are not answered in this version of the thesis.
...
\Theac{algebra} aims at answering the \cref{question:ordering-representation}.
...
\Cref{fig:strategy-overview} shows the strategy overview.
...
The fault events sequences are then mapped to \theac{algebra}, which have denotational semantics based in sets of lists.
...
Each predicate in \ac{activation} generates an expression in \theac{algebra}, which are reduced to obtain a canonical form to obtain \acp{MCSeq} and to calculate top-events probability.
...
\begin{alineas}
  \item Define a denotational and algebraic model to express fault events order with \theac{algebra}, and formally verify \acg{FT} (\ac{TFT} and \ac{DFT}) acceptance;
  ...
  \item Illustrate the application of the laws on \hl{two}\adnote{Pensei em usar mais estudos de caso, se der tempo, claro.} real case studies, provided by our industrial partner, \EMBRAER.
\end{alineas}

Other contributions of this work are:

\begin{alineas}
  \item Define a new operator to express order explicitly and proving that the resulting algebra---(\ac{algebra}) using this operator and Boolean\index{Boolean Algebra} operators---is a conservative extension of the Boolean algebra\index{Boolean Algebra} (also published in~\cite{DM2016});
  \adnote{When accepted, add work submitted to ISF}
  \item Generalise laws in~\ac{algebra} in terms of abstract properties, similar to healthiness conditions in \theac{UTP}.
\end{alineas}

We use Isabelle/HOL\index{Isabelle/HOL} 2015\footnote{The 2002 tutorial is reported in~\cite{NPW2002}, but there is a newer version published with the tool itself.
...
\Cref{part:contributions} describes the results: \cref{sec:strategy} presents our strategy, \cref{sec:case-study} the case study and the application of the proposed strategy, and we present our conclusions and future work in \cref{sec:conclusion}.
...
\hl{Means to dependability}\amnote{Está muito filosófico isto. É assim mesmo ou seja, não há uma definição para dependebilidade?} \adnote{Eu explico dependabilidade a seguir.} are obtained by modelling and analysing a system.
...
The concepts that creates the basis for dependability\index{dependability} are:
\begin{alineasinline}
  \item threats to,
...
\begin{description}
  ...
  \item[Integrity:] absence of improper systems \hl{alterations}\adnote{Em \cite{ALR+2004}, se usa essa expressão ``alterations'', mas acredito que seja sinônimo de ``change'', como mostrado em \url{http://www.merriam-webster.com/thesaurus/alteration}.};
...
The most traditional gates are \ac{AND} and \ac{OR}, which are equivalent to Boolean operators.
...
The idea was to create easy-to-use and \hl{less error-prone modelling tools}\amnote{Interessante... Quer dizer que usar DTMCs causa erros de modelagem?? Ainda hoje? Qual o artigo que cita isto?} \adnote{Bom... como DTMCs não mudaram, as respostas são: sim, sim, o de Dugan na sentença anterior} than using \acp{DTMC}---or simply \emph{\acap{DTMC}}---directly.
...
There are two ways to generate \iac{BDD} for an expression:
\begin{alineasinline}
...
A backup system in \ac{CSp} gets activated \hl{slower}\amnote{ou lower?} \adnote{``activation'' e ``readiness'' são coisas diferentes: a ativação pode ser lenta e a prontidão pode ser baixa.} than in \ac{WSp}, which implies that there are less failure possibilities in \ac{CSp}, but its the readiness is \hl{lower} than in \ac{WSp}.
...
This definition is more elegant because it represents a Boolean\index{Boolean Algebra} algebra as an algebra of sets and does not rely on axioms (which can be misleading, case there is an unfounded axiom).
...
\begin{Prop}[Soundness and completeness of \ac{algebra}]
...
When reasoning about faults, the engineer analyse component by component, defining its outputs in the presence of each possible fault.
The \ac{activation} is nothing more than this: the output of the components in the presence of (some combination of) faults.
To ensure that all possibilities are perfectly passed through (the engineer reasoned about them), the formula that results from the output conditions shall be a \emph{tautology}.
For example, if the engineer defines that a component has output:
\begin{alineasinline}
...
To connect components and to generate \acp{FT}, we ask questions to the \ac{activation} formulas: we define a \emph{predicate}.

A nominal value is required to handle the conditions that do not result in a failure.
In the previous example, if $F_1$ and $F_2$ do not occur, then the system should be in a normal state, and its output is signalled as nominal with some nominal value.
...
For example: what are the conditions that generates and output \emph{omission}?
...
\begin{Prop}[Soundness and completeness of \ac{activation}]
...
\section{Structure expressions with \ac{XBefore}}
...
\IBGEtab{%
  ...
  \begin{tabular}{p{10.1cm}ccc}
  \toprule
    Task & Start & Days & Days total \\
  \midrule \midrule
    Qualification & 04/04/2016 & 1 & 0 \\
  \midrule
    Elaborate theory for the \ac{activation} & 05/04/2016 & 45 & 1 \\
  \midrule
    Elaborate theory for the acceptance criteria & 20/05/2016 & 30 & 46 \\
  \midrule
    Prepare paper about \ac{activation} and acceptance criteria & 19/06/2016 & 30 & 76 \\
  \midrule
    Submit paper about \ac{activation} and acceptance criteria & 19/07/2016 & 1 & 106 \\
  \midrule
    Prove soundness and completeness theorems for the \ac{DNF} of \ac{algebra} & 20/07/2016 & 75 & 107 \\
  \midrule
    Define the mapping rules from traces to \ac{algebra} & 03/10/2016 & 15 & 182 \\
  \midrule
    Demonstrate the relations of \ac{NOT} and \ac{XBefore} and other operators & 18/10/2016 & 15 & 197 \\
  \midrule
    Define the conditions that cause non-coherent analysis with \ac{NOT} & 02/11/2016 & 15 & 212 \\
  \midrule
...
The next steps in this thesis are the conclusion of the work-in-progress of the ``Faults Modelling'' and ``Automatic FTA'' blocks in \cref{fig:status-of-thesis}.
...
We showed the \ac{DNF} for \ac{algebra}, but did not demonstrate that every formula can be converted into \ac{DNF}.
...
Considering that \acp{DFT} were conceived as a visual representation of \acap{DTMC}, then we may say that \acp{DFT} can be used to represent several states, thus they are suitable to propose the architectural model modifications as shown in \cref{fig:strategy-overview,fig:status-of-thesis}.
The definition and the theory of ``Faults Modelling and Fault Tolerance Patterns'' and the automatic proposal of ``Architectural Model Modifications'' blocks are left as future work.
