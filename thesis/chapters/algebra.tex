...
We use the concept of generators of the set of sets of \acp{FBA}\index{Boolean Algebra!Free} to propose the \ac{algebra} with a denotational semantics of a set of lists without repetition (\distinctlists).
...
\denote{\var{x}} =
  \left\{~
    zs ~\middle|~ \exists~ zs ~\bullet~ x \in zs
  ~\right\}
\end{equation}
%
...
Given this definition, we show a small example of how the \ac{XBefore} operator works:
%
\begin{align}
...
\]
%
and $\generators(\UNIV) = \left\{a,b\right\}$.
...
Expressions \cref{def:algebraset-var} to \cref{def:algebraset-sup} and \cref{eqs:generators-independence-eq-eq} to \cref{eqs:generators-independent-compl-subseteq-neq} implies that the \ac{algebra} without the \ac{XBefore} operator \cref{def:xbefore-append} forms a Boolean\index{Boolean Algebra} algebra based on sets of lists.
...
We informally demonstrate that the property $\tempo[1]{}$ is satisfied by the above set:
\begin{align}
...
The absence of occurrences ($\emptyset$, the empty set of $\algebraset$) is a ``0'' for the \ac{XBefore} operator (\cref{thm:xbefore-of-false-1,thm:xbefore-of-false-2}).
...
To allow formula reduction we need to relate \ac{XBefore} to the other Boolean\index{Boolean Algebra} operators.
...
The \ac{XBefore} is transitive with preconditions over the intermediate variable (\cref{thm:inf_xbefore_trans}).
Lastly, the \ac{AND} is absorbed with an \ac{XBefore} (\cref{thm:xbefore_inf_absorb_1,thm:xbefore_inf_absorb_2}).

Applying \cref{thm:xbefore-inf-1,thm:xbefore-inf-2} in \cref{thm:and_xbefore_equiv_or_xbefore} results in:
%
%\begin{subequations}
\begin{align}
...
%\begin{align}
%\semanticsof{sFalse} = & \semanticsof{\bot} \nonumber\\
%%%%%%
%\semanticsof{sVar~a} = & \semanticsof{\var{a}} \nonumber\\
%%%%%%
...
$\syntactically{\Sigma}{f}$ means that $f$ can be syntactically proved by the rules in $\Sigma$, whereas $\semantically{\Sigma}{f}$ means the same at the (denotational) semantics level.
...
\end{alineasinline}
%
These attributes of \iac{FT} are the most representative ones, but others can be modelled similarly.

\subsection[Minimal cut sequence]{\Acl{MCSeq}}

Recall from the beginning of this \lcnamecref{chap:algebra} that the denotational semantics of a formula in \ac{algebra} is a set of \distinctlists.
...
%
\begin{equation}
\mcseq{S} = \left\{~ xs ~\middle|~ xs \in \denote{S} \land \length{xs} = min_S ~\right\}
\end{equation}
%
where
\[
...
\]
%
and $\Minop$ returns the size of the smallest sequence in the given set.
\end{definition}
...
For \ac{algebra}, our proposal for probability calculation is defined in terms of the probability of \ac{PAND} gates, as shown in \cref{eq:general-pand-probability}.
...
Using \cref{eq:general-pand-probability,eq:generators} we define the probability for a list of faults.

\begin{definition}[Probability of a list of faults in \ac{algebra}]
\label{eq:denotational-probability}
Let $xs$ be a list of faults. Then the probability of $xs$, $\denotationalprob{xs}$, is given by:
%
\begin{equation}
\denotationalprob{xs} = 
\probability{xs} \times \prod_{f_j ~\in~ \generators - \listset{xs}} \left(1-P_j\left(t\right)\right)
\end{equation}
...
As the probability of each fault sequence is independent of each other, the probability of \iac{algebra} formula is the sum of the probabilities of its constituent lists of faults.
...
In \ref{eq:prob-xbefore-sup-equiv-inf} we demonstrated that the probability of a formula $\left(\xbefore{\var{f_1}}{\var{f_2}}\right) \lor \left(\xbefore{\var{f_2}}{\var{f_1}}\right)$ is equal to the probability of the traditional \ac{AND} probability ($\var{f_1} \land \var{f_2}$, \cref{eq:traditional-and-probability}).
...
\end{alineasinline}
See more examples in~\cite[p. XI-5,XI-18]{VGR+1981}.
...
\end{subequations}
%
Both equations return a Boolean\index{Boolean} value, which can be verified by a theorem prover.
...
Thus, the order-related information was lost, as there are no means to represent these failure traces in Boolean algebra\index{Boolean algebra}.
