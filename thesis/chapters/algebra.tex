...
We use type classes in Isabelle/HOL to describe the syntax (operators, also called \emph{parameters}) and the axioms (\emph{assumptions}) of \ac{algebra}.
%
The instantiation of the type class maps each operator to a set of \distinctlists, including the operators of Boolean algebra and the \ac{XBefore} operator.
%
Such an instantiation requires proof obligations for the type (the set of \distinctlists), which we discharged.
%
With all proofs, Isabelle/HOL asserts that the declared type, the set of sets of \distinctlists (all possible semantics) is indeed \iac{algebra}\footnote{
...
Soundness in the concrete syntax of Isabelle/HOL is a direct consequence of such an instantiation.
...
To prove soundness in the abstract syntax of Isabelle/HOL, we declare a 
datatype:
%
\begin{verbatim}
...
The semantics of a formula in the abstract syntax is similar to what we show in this \lcnamecref{chap:algebra}:
\begin{align}
...
Note that we mapped the semantics of a formula in the abstract syntax to the concrete syntax.
This is possible due to the locales and contexts in Isabelle/HOL.
Type classes use both structures of locales and contexts, thus it allows to reuse the concrete syntax, as we show above.
...
The syntactical application of rules can be defined in Isabelle/HOL as an inductive definition.
Thus, if a formula is in ($\in$) such an inductive definition, then such a formula is provable.
