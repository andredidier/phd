...
The resulting combinations for $TOP$ are $\left\{a,b\right\}$ and $\left\{a,b,c\right\}$ (the system has three fault events $a$, $b$, and $c$, and the two sets represent the occurrence of fault events $a$ and $b$, with $c$'s occurrence or without it).
...
These expressions use the \ac{SFT} operators \ac{OR} and \ac{AND}, and three new operators\footnote{In formulas, the following symbols are used to represent the operators \ac{PAND}, \ac{POR}, and \ac{SAND}, respectively: ``$<$'', ``$|$'', and ``$\&$''} related to events ordering: \acf{PAND}, \acf{POR}, and \acf{SAND}.
