...
We were inspired by \ac{FBA} concepts to create our \acl{algebra} (\cref{chap:algebra}).
We explain \acp{FBA} in \cref{sec:fba}.

The quantitative analysis of \acp{FT} requires a probability theory of fault events.
It is introduced in~\cref{sec:probability-theory}.
...
In order for the pumps unit to fail, all three pumps need to fail, and the left-hand side spare gate needs to fail before (or at the same time as) the right-hand side spare gate (\ac{PAND} gate\footnote{Although the original example uses \iac{PAND} gate, according to the informal description, \iac{SEQ} gate would fit better.}).
...
%\input{chapters/sections/structure-expression-analysis}
...
\section{Probability theory of fault events}
\label{sec:probability-theory}

The work reported in~\cite{VGR+1981} presents the mathematical description of events.
It describes three kinds of events:
\begin{alineasinline}
	\item independent events,
	\item mutually exclusive events, and
	\item dependent events.
\end{alineasinline}
%
Independent events are those as discussed in \cref{sec:time-relations}.
Their occurrence and duration vary independently.
The mutually exclusive events are the kind of events that do not happen at the same time at all.
Dependent events are those in which the occurrence of one implies the occurrence of the other.

In the context of \acp{FT}, the combined probability of the \ac{OR} gate, $\probability{Q_{\mathrm{OR}}}$, for fault events $A$ and $B$, is:
\begin{equation}
\label{eq:traditional-or-probability}
\probability{Q_{\mathrm{OR}}} = \probability{A} + \probability{B} - \probability{A \inter B}
\end{equation}
%
where $\probability{A \inter B}$ is the probability of $A$ given $B$ has occurred, $\probability{A|B}$, or the probability of $B$ given $A$ has occurred, $\probability{B|A}$ (if the events are random, these probability values are the same).

Considering the relations of fault events:
\begin{itemize}
	\item If $A$ and $B$ are independent events, then $\probability{Q_{\mathrm{OR}}} = \probability{A} + \probability{B} - \probability{A} \times \probability{B}$;
	\item If $A$ and $B$ are mutually exclusive events, then $\probability{Q_{\mathrm{OR}}} = \probability{A} + \probability{B}$; and
	\item If $B$ is completely dependent on event $A$ (whenever $A$ occurs, $B$ also occurs), then $\probability{Q_{\mathrm{OR}}} = \probability{B}$.
\end{itemize}

The combined probability of the \ac{AND} gate, $\probability{Q_{\mathrm{AND}}}$, for fault events $A$ and $B$, is:
%
\begin{equation}
\label{eq:traditional-and-probability}
\probability{Q_\mathrm{AND}} = \probability{A} \times \probability{B|A} = \probability{B} \times \probability{A|B}
\end{equation}

Considering the relations of fault events:
\begin{itemize}
	\item If $A$ and $B$ are independent events, then $\probability{Q_\mathrm{AND}} = \probability{A} \times \probability{B}$; 
	\item If $A$ and $B$ are mutually exclusive events, then $\probability{Q_\mathrm{AND}} = 0$; and
	\item If $B$ is completely dependent on event $A$, then $\probability{Q_\mathrm{AND}} = \probability{A}$
\end{itemize}
...
\end{align}
%
where $P_1$ and $P_2$ are the probabilities of the occurrences (cumulative distribution function) of the first and the second faults, respectively, and $T_1$ and $T_2$ are the times the first and the second faults occur.
...
Valve $VAL$ closes a gas flow if a sensor detects a leak $L$ in room 1.
...
In room 1 there is a possible ignition source ($I1$), and
nearby there is a permanent source of ignition ($I2$).
