...
Positive requirements such as, for example, ``the communications system should be operational $99.99\%$ of the cruise phase'' are treated as a complement (the complete failure should have a probability in less than $0.01\%$ of the cruise phase).
...
The \ac{activation} associates each operational mode with a faults expression.
...
Given the flexibility of the \ac{activation} notation, it can be used to reason about basic fault events and top-event failures, which are related to~\ref{question:ordering-representation}.
...
\begin{contrenum}[series=contributions]
  ...
  \item Reason about fault modelling in \ac{activation} to obtain formal expressions of critical failures (top-event failures, \cref{chap:activation});
    %%%%%
  \item Illustrate the application of the laws of \ac{algebra}, the use of \ac{activation}, and the acceptance criteria verification on a real case study, provided by \embraer (\cref{chap:case-study}), and on a literature case study.
