theory FaultTree
imports Main FaultModellingTypes BoolOperand Complex_Main
begin
...
primrec GateToOp :: "FaultTreeGate \<Rightarrow> ('vb BoolOperand binop)"
where
  "GateToOp FTGAnd = (\<lambda> a b. VBBAndOp a b)" |
  "GateToOp FTGOr = (\<lambda> a b. VBBOrOp a b)"

primrec FT_to_BO :: "'vb FaultTree \<Rightarrow> 'vb BoolOperand"
where
...
(*
theorem "(BoolOperand_eval (FT_to_BO T) = (BoolOperand_eval B)) \<longrightarrow> 
  (FaultTree_eval T vb = BoolOperand_eval B vb)"
apply (auto)
apply (auto simp add: FT_to_BO_def)
apply (auto simp add: FaultTree_eval_def)
apply (auto simp add: BoolOperand_eval_def)
